...
echo Html::tag('h1',Html::encode($this->title));
    ...
    if(Yii::$app->session->hasFlash('herstellerDataUpdated')){
        echo Html::beginTag('div');
        echo Alert::widget([
            'options' => ['class' => 'alert-info'],
            'body' => Yii::$app->session->getFlash('herstellerDataUpdated'),
        ]);
...
$( document ).ready(function() {
    ...
    $('#someSwitchOptionDefaultArtikeltyp').change(function() {   
        if($('#someSwitchOptionDefaultArtikeltyp').prop('checked')){
           $('#textinputNewArtikeltyp').show();  
           $('#dropdownArtikeltyp').hide();
        }
        else{
           $('#textinputNewArtikeltyp').hide();
           $('#dropdownArtikeltyp').show();
        }               
    });  
});
JS;
